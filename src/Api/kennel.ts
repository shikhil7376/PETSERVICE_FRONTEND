...
export const signup = async(userData:kennelFormData)=>{
    try {
...
export const resendOtp = async(userData:loginData)=>{
    try {
...
export const cancelBooking = async(bookingid:string,cageid:string)=>{
    try {
        const response = await api.post(kennelRoutes.cancelBooking,{bookingid,cageid})
        return response
    } catch (error) {
        return errorHandle(error)
    }
