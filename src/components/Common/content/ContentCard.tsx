...
const ContentCard: React.FC<ContentCardProps> = ({ post, fetchData, fetchNotFollowData, showButton }) => {
  ...
  useEffect(()=>{
    const fetchReportStatus = async ()=>{
      try {
        const response = await checkReportStatus(post.id,userData?._id as string)
          if(response){
            setReported(response.data.data)
          }
      } catch (error) {
        errorHandle(error)
      }
  ...
  return (
    <Card className="flex-none z-30 w-full md:w-[420px] md:h-[400px] bg-black p-2 ">
      <CardHeader className="justify-between">
        ...
        {post?.user?.userid !== userData?._id && showButton && (
      ...
      <div className=' display flex gap-2 px-2'>
        <p className='text-gray-400 font-roboto text-sm'>{post.description}</p>
      </div>
